...
class StuNet(nn.Module):
    ...
    def __init__(self, model="resnet32", num_branches=4, num_classes=10, input_channel=64, en=False, factor=8, dropout=0.0):
        ...
        for i in range(num_branches):
            if model == "resnet32":
                setattr(self, 'stu'+str(i),
                        resnet32(num_classes=num_classes, KD=True))
            elif model == "resnet110":
                setattr(self, 'stu'+str(i),
                        resnet110(num_classes=num_classes, KD=True))
            elif model == "vgg16":
                setattr(self, 'stu'+str(i),
                        vgg16(num_classes=num_classes, KD=True, dropout=dropout))
            elif model == "densenetd40k12":
                setattr(self, 'stu'+str(i),
                        densenetd40k12(num_classes=num_classes, KD=True))
            elif model == "wide_resnet20_8":
                setattr(self, 'stu'+str(i),
                        wide_resnet20_8(num_classes=num_classes, KD=True))
    ...
    def forward(self, x):
        # (B X 64), (B X num_classes)
        x_f, pro = self.stu0(x)
        # B x input_channel//factor
        proj_query = self.query_weight(x_f)
        # B x input_channel//factor
        proj_key = self.key_weight(x_f)
        ...
        if self.en:
            for i in range(1, self.num_branches):
                ...
                # B x input_channel//factor
                x_f_q = self.query_weight(temp_x_f)
                # B x input_channel//factor
                x_f_k = self.key_weight(temp_x_f)
