...
class kd_loss_fn(nn.Module):
    ...
    def forward(self, output_batch, labels_batch, teacher_outputs):
        ...
        KL_teacher = nn.KLDivLoss(reduction='batchmean')(
            output_batch, teacher_outputs) * self.T
...
class Att_Loss(nn.Module):
    ...
    def forward(self, output_batch, labels_batch, attention):
        ...
        # B X num_student
        labels_batch = labels_batch.view(-1, 1).repeat(1, num_student)
        ...
        attention_label = torch.bmm(output_batch, attention.permute(
            0, 2, 1))     # B X num_classes X num_student

        if self.loss == 'CE':
            output_batch = F.log_softmax(output_batch/self.T, dim=1)
            # B X num_classes X num_student
            attention_outputs = F.softmax(attention_label/self.T, dim=1)
            loss_att = -torch.sum(torch.mul(output_batch,
                                            attention_outputs))/batch_size
        elif self.loss == 'MSE':
            # calculate the average distance between attention and identity
            output_batch = F.softmax(output_batch, dim=1)
            # B X num_classes X num_student
            attention_outputs = F.softmax(attention_label, dim=1)
            loss_att = torch.sum(
                (output_batch - attention_outputs) ** 2) / batch_size
        # calculate the log angle
        identity = torch.eye(num_student).reshape(
            1, num_student, num_student).repeat(batch_size, 1, 1).cuda()
        # calculate the average distance between attention and identity
...
class E_Loss(nn.Module):
    ...
    def forward(self, output_batch, teacher_outputs):
        ...
        # Same result CE-loss implementation torch.sum -> sum of all element
        loss = -self.T*self.T * \
            torch.sum(torch.mul(output_batch, self_outputs)) / \
            output_batch.size(0)
